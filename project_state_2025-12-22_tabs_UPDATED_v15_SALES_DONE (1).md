...
#### تحديث جلسة اليوم — فاتورة المبيعات (Sales Invoice)

**الهدف:** قبل إضافة سطر البيع، يتم جلب بيانات الصنف تلقائيًا (الكميات + تشغيلات FEFO + الخصم المرجّح + خصم البيع النهائي) مع الحفاظ على مبدأ **السرعة** وعدم السماح بتعديل حقول لا يجب تعديلها.

**ما تم تثبيته اليوم:**

- **الخصم المرجّح للصنف % (Weighted Purchase Discount):**
  - يتم حسابه من خدمة التحليل (StockAnalysisService) ويُعرض في كارت بيانات الصنف.
  - يتم إرجاعه ضمن JSON من `GetSalesProductInfo` في `SalesInvoicesController`.

- **خصم البيع النهائي % (Auto):**
  - تمت إضافة منطق خصم البيع بحيث:
    - العميل له Policy (لو العميل بدون سياسة → نثبّت له PolicyId=1 افتراضيًا).
    - نقرأ "مكسب المخزن" من جدول قواعد سياسة المخزن (WarehousePolicyRule) حسب (PolicyId + WarehouseId).
    - خصم البيع النهائي = `الخصم_المرجّح - مكسب_المخزن`.
    - مع حماية: لا يقل عن 0 ولا يزيد عن 100.
  - تم اعتماد الدالة:
    - `StockAnalysisService.GetSaleDiscountAsync(prodId, warehouseId, customerId, weightedPurchaseDiscount)`

- **تمرير CustomerId إلى السيرفر:**
  - في JS داخل صفحة فاتورة المبيعات، تم إرسال `customerId` مع `warehouseId` إلى `GetSalesProductInfo`:
    - `...&customerId=...&warehouseId=...`
  - وبالتالي أصبح خصم البيع يعتمد فعليًا على **العميل + المخزن + الخصم المرجّح**.

- **قفل الحقول غير القابلة للتعديل:**
  - السعر في المبيعات **ليس اختياريًا** → دائمًا ReadOnly (يأتي من تشغيلات FEFO).
  - تم إضافة قفل/فتح لخصم البيع بالضغط على لابل الخصم (مثل فكرة خصم المشتريات):
    - افتراضيًا مقفول.
    - عند الفتح: Focus + Select.
    - عند الغلق: Enter يتخطاه.

- **Enter Navigation ذكي (لا يمر على المقفول):**
  - مسار Enter يعتمد على ترتيب ثابت للحقول.
  - Enter ينتقل إلى **أول حقل مفتوح** بعد الحالي (يتخطى `readOnly/disabled/hidden`).
  - عند تغيير قفل الخصم (فتح/غلق) يتم استدعاء `rebuildEnterNav()` لتحديث المسار فورًا.

**ملاحظات مهمة للثوابت:**
- لا يوجد شيء اسمه “السعر اختياري” في البيع.
- خصم البيع يتم حسابه قبل إضافة السطر ويظهر في البوكس تلقائيًا (مع إمكانية فتح/تعديل يدوي عند الحاجة عبر اللابل).
