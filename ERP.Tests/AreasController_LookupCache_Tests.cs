...
public class AreasController_LookupCache_Tests
{
    ...
    private static AreasController CreateController(AppDbContext db, ILookupCacheService lookupCache)
    {
        ...
        var controller = new AreasController(db, activityLogger.Object, lookupCache);
    ...
    [Fact]
    public async Task Create_PopulatesGovernoratesAndDistrictsFromLookupCacheUntilTheyAreCleared()
    {
        ...
        Assert.IsType<Area>(firstResult.Model);
        ...
        await controller.Create();
        Assert.Equal(2, ReadSelectTexts(controller.ViewBag.GovernorateId).Length);
        Assert.Equal(3, ReadSelectTexts(controller.ViewBag.DistrictId).Length);
    }
}
