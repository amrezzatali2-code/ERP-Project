...
- **المطلوب النهائي للشكل:** الأزرار تكون **جانبية على اليمين** (مثل شاشة "طلب شراء").
- **السلوك:** الشريط الجانبي **ثابت أثناء السكرول** (Sticky) ولا يختفي.
- **Tooltips:** إزالة الـ tooltip من كل الأزرار الجانبية، **ويُترك فقط** على:
  - أسهم التنقل (أول/سابق/التالي/آخر)
  - مربع البحث وزر البحث
- **ملاحظة مشكلة ظهرت:** بعد تعديل وضع الشريط ليصبح جانبي/ثابت، أصبحت "فاتورة جديدة" تفتح أحيانًا **فارغة**.
  - غالبًا السبب **HTML structure** (أقفال div غير متوازنة) أو نقل جزء من الـ Razor block داخل container خطأ.
  - في الجلسة القادمة سنراجع: ترتيب الـ divs حول `invoice-page` و `container-fluid` و `invoice-actions-bar`،
    والتأكد أن محتوى الفاتورة (الهيدر + الكروت + السطور) داخل نفس العمود الرئيسي ولم يخرج خارج `row`.
- **الحالة الحالية:** الشكل المطلوب تم الوصول له جزئيًا (الشريط يمين) لكن ما زالت مشكلة "فتح الفاتورة فاضية" قائمة.
