...
## ⚡ ملاحظة أداء (أبريل 2026)

### بطء فتح القوائم بعد تعديلات الكاش/الصلاحيات

- تم رصد بطء واضح في صفحات القوائم (`Index`) بسبب تكرار فحص صلاحيات `Global.*` عبر DB عند كل طلب.
- السبب كان في `PermissionService.HasPermissionAsync(int userId, string permissionCode)` حيث لم يكن يعتمد على كاش الصلاحيات أولاً بنفس كفاءة المسار الآخر.
- الحل المطبق:
1. إضافة مسار سريع يعتمد على `GetUserPermissionCodesAsync(userId)` قبل أي استعلامات DB.
2. دعم توافق الأكواد القديمة (legacy aliases) مباشرة من الكاش.
3. إرجاع سريع `false` لصلاحيات `Global.*` إذا لم تظهر في الكاش، لتقليل الاستعلامات المتكررة في `ListVisibilityService` و`PopulateUserPermissionsFilter`.

### ملفات مرتبطة

- `Services/PermissionService.cs`
- `Filters/PopulateUserPermissionsFilter.cs`
- `Services/ListVisibilityService.cs`

---

## 🧾 ملاحظة واجهة (أبريل 2026)

### شاشة فاتورة المبيعات `SalesInvoices/Show`

1. زر **مرتجع جديد باسم العميل** أصبح يتفعّل بمجرد وجود رقم فاتورة (`SIId > 0`) حتى لو لم تُحمّل قيمة `CustomerId` فورًا في الـ DOM.
2. زر **حفظ الفاتورة** (`btnSaveInvoiceHeader`) أصبح يُقفل بعد أول حفظ ناجح لمنع الضغط المتكرر غير الضروري.
3. تحديث 2026-04-20: قفل زر الحفظ أصبح فقط بعد أول ضغطة ناجحة تُنشئ رقم الفاتورة (عند الانتقال من `SIId=0` إلى رقم فعلي).
4. تحديث 2026-04-20 (طباعة أصل وصورة): توحيد حجم كروت الإجماليات، مع إزالة كارت **المبلغ المتبقي من الحد الائتماني** وكارت **عدد السطور** من إجماليات الطباعة.

### توحيد نظام الطباعة (2026-04-20)

1. شاشة **مرتجع المبيعات** `SalesReturns/Show`:
- إصلاح ربط أحداث قائمة الطباعة بحيث تعمل بثبات عبر تفويض الحدث (`data-sr-print-mode`) بدل الربط المباشر للعناصر.
2. شاشة **فاتورة المشتريات** `PurchaseInvoices/Show`:
- تحويل زر الطباعة إلى قائمة منسدلة مثل فاتورة المبيعات: `صفحة واحدة / أصل وصورة / أصل وصورتين`.
- توحيد التنفيذ ليستخدم `print-mode-1/2/3` مع إنشاء نسخ ديناميكيًا قبل الطباعة وإرجاع DOM بعد `afterprint`.
3. شاشة **طلب الشراء** `PurchaseRequests/Show`:
- نفس نظام فاتورة المشتريات أعلاه (قائمة منسدلة + نسخ + استرجاع DOM بعد الطباعة).
4. شاشة **مرتجع المشتريات** `PurchaseReturns/Show`:
- إضافة نفس قائمة أنماط الطباعة (صفحة واحدة/أصل وصورة/أصل وصورتين) مع تجهيز نسخة طباعة من منطقة العمل الرئيسية قبل `window.print`.
